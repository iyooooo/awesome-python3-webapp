...
async def logger_factory(app, handler):
	async def logger(request):
		logging.info('\n\n\nlogger_factory-------->: %s %s ' % (request.path, handler))
...
def auth_factory(app, handler):
	@asyncio.coroutine
	def auth(request):
		logging.info('auth_factory------->: %s %s ' % (request.path, handler))
...
async def data_factory(app, handler):
	async def parse_data(request):
		logging.info('data_factory json:%s' % request.method)
...
async def response_factory(app, handler):
	async def response(request):
		logging.info('response_factory  --------> beigin:%s  path %s  %s' % (request.method, request.path, handler))
		r = await handler(request)
		if  isinstance(r, web.StreamResponse):
			logging.info('response_factory StreamResponse --------> ')
			return r
		if isinstance(r, bytes):
			logging.info('response_factory bytes --------> ')
			resp = web.Response(body=r)
			resp.content_type = 'application/octet-stream'
			return resp
		if isinstance(r, str):
			logging.info('response_factory --------> str')
		...
		if isinstance(r, dict):
			logging.info('response_factory  --------> dict')
		...
		if isinstance(r, int) and r >= 100 and r < 600:
			logging.info('response_factory int --------> %s', r)
			return web.Response(r)
		if isinstance(r, tuple) and len(r) == 2:
			logging.info('response_factory tuple --------> %s', r)
			t, m = r
