...
async def logger_factory(app, handler):
	async def logger(request):
		# logging.info('logger_factory: %s %s ' % (request, handler))
...
async def response_factory(app, handler):
	async def response(request):
		# logging.info('--------> response_factory:%s', request)
		...
		if isinstance(r, bytes):
			logging.info('response_factory bytes --------> %s', r)
			resp = web.Response(body=r)
			resp.content_type = 'application/octet-stream'
			return resp
		if isinstance(r, str):
			logging.info('response_factory --------> str %s', r)
		...
		if isinstance(r, dict):
			logging.info('response_factory  --------> dict %s', r)
